...
def main():
    ...
    print("listening on 0.0.0.0:%d (udp)" % port)
    ...
    while True:
        ...
        print("connection from %s:%s" % (addr, data))
        if data[:4] == "msg ":
            ...
            try:
                print('normal message', symmetric_chat_clients)
                sockfd.sendto('不要回答'.encode(), addr)
                sockfd.sendto('不要回答2'.encode(), addr)
                pool, nat_type_id = data.strip().split()
                sockfd.sendto("ok {0}".encode(), addr)
