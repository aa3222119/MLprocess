<?php

function transformKey($key, $effKeyLen)

{
    $pitable = array(
        );
    $invpitable = array_flip($pitable);

    //    Apply the regular RC2 key expansion algorithm.
    $t = strlen($key);
    $key = array_values(unpack('C*C', $key));

    for ($i = $t; $i < 128; $i++)
        $key[$i] = $pitable[($key[$i - 1] + $key[$i - $t]) & 0xFF];

    $t8 = ($effKeyLen + 7) >> 3;
    $tm = 0xFF >> (8 * $t8 - $effKeyLen);
    $i = 128 - $t8;
    $key[$i] = $pitable[$key[$i] & $tm];

    while ($i--)
        $key[$i] = $pitable[$key[$i + 1] ^ $key[$i + $t8]];

    //    Map the first byte: this operation will be undone by
    //        mcrypt internals.
    $key[0] = $invpitable[$key[0]];

    //    Return the transformed key as a string.
    array_unshift($key, 'C*');
    return call_user_func_array('pack', $key);
}


class aes {
 
    // CRYPTO_CIPHER_BLOCK_SIZE 32
     
    private $_secret_key = 'default_secret_key';
     
    public function setKey($key) {
        $this->_secret_key = transformKey($key,32);
        var_dump($this->_secret_key);
    }
     
    public function encode($data) {
        $td = mcrypt_module_open(MCRYPT_RIJNDAEL_256,'',MCRYPT_MODE_ECB,'');
        var_dump(mcrypt_enc_get_key_size($td));
        $iv = mcrypt_create_iv(mcrypt_enc_get_iv_size($td),MCRYPT_RAND);
        var_dump($iv);
        mcrypt_generic_init($td,$this->_secret_key,$iv);
        
        $encrypted = mcrypt_generic($td,$data);
        mcrypt_generic_deinit($td);
         
        return $iv . $encrypted;
    }
     
    public function decode($data) {
        $td = mcrypt_module_open(MCRYPT_RIJNDAEL_256,'',MCRYPT_MODE_ECB,'');
        $iv = mb_substr($data,0,32,'latin1');
        mcrypt_generic_init($td,$this->_secret_key,$iv);
        $data = mb_substr($data,32,mb_strlen($data,'latin1'),'latin1');
        $data = mdecrypt_generic($td,$data);
        mcrypt_generic_deinit($td);
        mcrypt_module_close($td);
        return trim($data);
    }
}
 
$aes = new aes();
//'8bca64bbc90b352790b46a37a77d740c'
//'8bca64bbc90b352790b46a37a77d740cf52c59f42ad2a41810508fe69ed3f251'
$aes->setKey('8bca64bbc90b352790b46a37a77d740cf52c59f42ad2a41810508fe69ed3f251');

 
// 加密
$string = $aes->encode('<RESPONSE><RSCODE>01000081</RSCODE><RSMSG>优惠政策标识与增值税特殊管理，输入的条件不匹配!_FyxmN1</RSMSG><DATA><FPQQLSH>APPLSH20171107162222</FPQQLSH><FPLXDM>026</FPLXDM><FP_DM></FP_DM><FP_HM></FP_HM><KPRQ>000000</KPRQ><JYM></JYM><PDF_URL></PDF_URL><SP_URL></SP_URL></DATA></RESPONSE>');
// 解密
$hex_str=bin2hex($string);
var_dump('\n');
var_dump($hex_str);
$base_str = base64_encode($hex_str);
var_dump('\n');
var_dump($base_str);
$url_encode = urlencode($base_str);
var_dump($url_encode);
var_dump('\n');
var_dump($aes->decode($string));
?>